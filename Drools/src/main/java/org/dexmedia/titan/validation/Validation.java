...
public class Validation {
	...
	{
		if (itemId!=null) {
			String hql = "select ENTERPRISE_ACCT_ID FROM DIGITAL_CONTENT where ENTERPRISE_ITEM_ID='"+itemId+"'";
			List<String> acountList = new ArrayList<String>();
			acountList = session.createSQLQuery(hql).list();
			if (acountList.size() < 0 || acountList.isEmpty()) //it is a new acount
				return true;
			else
				return false;
		}
	...
	public boolean currentDateValidation(JSONObject input) 
	{
		...
		try{
		 ...
		 /*ZoneId id = ZoneId.of("America/Chicago"); //CST - America/Chicago
